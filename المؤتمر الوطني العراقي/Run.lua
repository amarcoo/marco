...
─────▀██▀─────
┊marco ‿ @XXUUU 
تم تطوير وبرمجة السورس من قبل مطور الُسورس عماركو
┊@XXUUU ‿ @XXUUU
#-------------------------------------------------------------------
]]
...
marco = Token:match("(%d+)")
redis:set(marco..":VERSION",1)
redis:set(marco..":SUDO_ID:",SUDO_USER)
redis:set(marco..":DataCenter:",'German')
redis:set(marco..":UserNameBot:",BOT_User)
redis:set(marco..":NameBot:",BOT_NAME)
redis:hset(marco..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3marco_INSTALL","Yes")
info = {}
...
─────▀██▀─────
┊marco ‿ @XXUUU
تم تطوير وبرمجة السورس من قبل مطور السورس عماركو
┊@XXUUU ‿ @XXUUU
-------------------------------------------------------------------
]]..'\027[0;32m')
create_config() 
...
marco = Token:match("(%d+)")
our_id = tonumber(marco)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(marco..":UserNameBot:")
SUDO_ID = tonumber(redis:get(marco..":SUDO_ID:"))
SUDO_USER = redis:hgetall(marco..'username:'..SUDO_ID).username
version = redis:get(marco..":VERSION")
DataCenter = redis:get(marco..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..marco..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(marco..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(marco..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(marco..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(marco..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور '
msg.TheRank = 'مطور البوت '
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(marco..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(marco..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء '
msg.TheRank = 'المنشىء '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(marco..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير ' 
msg.TheRank = 'مدير البوت ' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(marco..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن '
msg.TheRank = 'ادمن في البوت '
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(marco..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(marco..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(marco..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(marco..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(marco..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(marco..':WELCOME_BOT'),[[•~مـرحبآ آنآ بوت آسـمـي ]]..redis:get(marco..':NameBot:')..[[•~آختصـآصـي حمـآيه‌‏ آلمـجمـوعآت
...
if msg.GroupActive and redis:get(marco..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(marco..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(marco..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(marco..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.marco then 
if msg.text and ScriptFile.imarco then
for k, marco in pairs(ScriptFile.marco) do
local SearchText = msg.text:match(marco)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.imarco(msg,{msg.text:match(marco)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",marco.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dmarco then
if ScriptFile.dmarco(msg) == false then
return false
end
...
if Plug.marco then 
if msg.text and Plug.imarco then
for k, marco in pairs(Plug.marco) do
local SearchText = msg.text:match(marco)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(marco..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(marco..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.imarco(msg,{msg.text:match(marco)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",marco.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source marco")
end 

end
else
print("The File Script.lua Not Runing in The Source marco")
...
function tdcli_update_callback(data)
	...
	redis:srem(marco..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/basselabaza/marco/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/basselabaza/marco/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/basselabaza/marco/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/basselabaza/marco/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/basselabaza/marco/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/basselabaza/marco/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(marco..":SUDO_ID:")
redis:hset(marco..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(marco..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(marco..'mute_tgservice'..msg.chat_id_) then
	Del_msg(msg.chat_id_,msg.id_)
	end
	elseif msg.content_.ID == "MessageChatAddMembers" then
	if redis:get(marco..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(marco..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(marco..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(marco..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(marco..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(marco..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(marco..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(marco..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(marco..'group:add'..msg.chat_id_) then
	redis:set(marco..'group:name'..msg.chat_id_,msg.content_.title_)
	end
	end 
	
	elseif data.ID == "UpdateNewChat" then  
	if redis:get(marco..'group:add'..data.chat_.id_) then
	redis:set(marco..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(marco..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(marco..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(marco..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	marco..'group:name'..k,IDS.Title,
	marco..'num_msg_marco'..k,5,
	marco..'group:add'..k,true,
	marco..'lock_link'..k,true,
	marco..'lock_spam'..k,true,
	marco..'lock_webpage'..k,true,
	marco..'lock_markdown'..k,true,
	marco..'lock_flood'..k,true,
	marco..'lock_bots'..k,true,
	marco..'mute_forward'..k,true,
	marco..'mute_contact'..k,true,
	marco..'mute_document'..k,true,
	marco..'mute_inline'..k,true,
	marco..'lock_username'..k,true,
	marco..'replay'..k,true
	)
	redis:sadd(marco..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(marco..'username:'..ID,'username',user)
	redis:sadd(marco..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(marco..'username:'..ID,'username',user)
	redis:sadd(marco..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(marco..'username:'..ID,'username',user)
	redis:sadd(marco..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'•~** تم رفع آلنسـخه‏‏ آلآحتيآطـيه\n•~** حآليآ عدد مـجمـوعآتك هيه *'..redis:scard(marco..'group:ids')..'* ®️\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(marco..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(marco..'username:'..data.user_.id_).username
	...
	redis:hset(marco..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
	...
	local groups = redis:smembers(marco..'group:ids')
	...
	local pv = redis:smembers(marco..'users')
	local NumPvDel = 0
	for i = 1, #pv do
